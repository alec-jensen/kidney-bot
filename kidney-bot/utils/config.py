...
class Config:
    def __init__(self):
        if not os.path.exists("config.json"):
            logging.critical("Config file does not exist. Create sample? (y/n)")
            if input("> ") == "y":
                shutil.copyfile("config.sample.json", "config.json")
                logging.info("Sample config created. Please edit it. Come back when you are done.")
    ...
    def load(self) -> None:
        ...
        try:
            ...
            if isinstance(ownerid, list):
                ...
                self.owner_ids: set[int] | None = {convert_except_none(i, int) for i in ownerid if i is not None}
            ...
            self.error_channel: int | None = convert_except_none(self.conf_json.get("error_channel"), int, error=False)
            if self.error_channel is None:
                logging.warning("No error channel configured, errors will only be logged to console.")
            ...
            self.langfile: str = self.conf_json.get("langfile", "lang/en_us.yml")
            if self.langfile == "":
                self.langfile = "lang/en_us.yml"

            if not os.path.exists(self.langfile):
                raise FileNotFoundError(f"Language file {self.langfile} does not exist.")
            ...
            # Web dashboard OAuth (consumed by services/api, not the bot process itself).
            self.discord_client_id: str | None = self.conf_json.get("discord_client_id") or None
            self.discord_client_secret: str | None = self.conf_json.get("discord_client_secret") or None
            self.discord_redirect_uri: str | None = self.conf_json.get("discord_redirect_uri") or None
            self.session_secret: str | None = self.conf_json.get("session_secret") or None
            self.dashboard_frontend_url: str | None = self.conf_json.get("dashboard_frontend_url") or None

            # Internal bot<->API WebSocket link (instant config-cache invalidation).
            # Both must be set for the link to activate; see cogs/dashboard_link.py.
            self.api_internal_url: str | None = self.conf_json.get("api_internal_url") or None
            self.internal_api_secret: str | None = self.conf_json.get("internal_api_secret") or None
    ...
    def reload(self) -> None:
        ...
        if self.dbstring != self.conf_json["dbstring"]:
            raise ValueError("Database string changed, please restart kidney-bot.")
